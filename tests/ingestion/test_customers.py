...
def test_ingest_writes_one_row_per_customer(tmp_path: Path) -> None:
    source = write_csv(
        tmp_path / "customers.csv",
        "customer_id,first_name\nC001,Ana\nC002,Marco\n",
    )
    output = tmp_path / "customers.parquet"

    ingest_customers(source, output, clock=FIXED_CLOCK)

    table = pq.read_table(output)
    assert table.num_rows == 2
    assert table.column("customer_id").to_pylist() == ["C001", "C002"]
...
def test_ingest_adds_ingested_at_from_clock(tmp_path: Path) -> None:
    source = write_csv(tmp_path / "customers.csv", "customer_id,first_name\nC001,Ana\n")
    output = tmp_path / "customers.parquet"
...
def test_ingest_raises_on_missing_customer_id_value(tmp_path: Path) -> None:
    source = write_csv(
        tmp_path / "customers.csv",
        "customer_id,first_name\nC001,Ana\n,Marco\n",
    )
...
def test_ingest_raises_on_missing_customer_id_column(tmp_path: Path) -> None:
    source = write_csv(tmp_path / "customers.csv", "first_name\nAna\n")
    output = tmp_path / "customers.parquet"
