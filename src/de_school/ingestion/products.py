"""Ingest the products CSV source into raw Parquet.

Grain: one row per product_id (exact duplicate rows are dropped; the input
is not expected to contain updates to the same product_id with different
...
def ingest_products(
    ...
    *,
    ...
    """Read products.csv, validate, deduplicate, and write raw Parquet.

    Raises ValueError if the required product_id column is missing, or if
    any row has a missing/empty product_id — invalid rows fail the whole
